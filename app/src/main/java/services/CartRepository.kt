...
class CartRepository(context: Context) {
    ...
    fun increaseCartItemQuantity(cartId: Long, name: String, price: Float, shortdesc: String, image: String, star: Float, quantity: Int, foodID: Long) {
...
//            cv.put(CartContract.CartEntry.CART_ITEM_QTY, quantity)
    ...
    fun decreaseCartItemQuantity(cartId: Long, name: String, price: Float, shortdesc: String, image: String, star: Float, quantity: Int, foodID: Long) {
        val db = dbHelper.writableDatabase
        val query = "SELECT * FROM ${CartContract.CartEntry.CART_TABLE_NAME} WHERE ${CartContract.CartEntry.CART_ITEM_ID}='${cartId}';"
        val cursor = db.rawQuery(query, null);
        if (cursor.count > 0) {
...
//            cv.put(CartContract.CartEntry.CART_ITEM_QTY, quantity)
    ...
    fun readCartData(): MutableList<CartItem> {
        ...
        with(cursor) {
            while (moveToNext()) {
                val id = getLong(getColumnIndexOrThrow(CartContract.CartEntry.CART_ITEM_ID))
                ...
                val foodId = getLong(getColumnIndexOrThrow(FoodContract.FoodEntry.COL_ITEM_ID))
                cartList.add(CartItem(id, image, name, price, desc, star, quantity, foodId))
            }
        }
        cursor.close()
        return cartList
